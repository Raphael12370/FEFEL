...
if(hasError){showToast(‘⚠️ Preencha todos os campos corretamente.’);return;}
if(stocks.length===0){showToast(‘⚠️ Adicione pelo menos uma ação.’);return;}
state.stocks=stocks;
...
if(!k){showToast(‘⚠️ Digite sua chave’);return;}
state.apiKey=k;saveState();
$(‘api-key-input’).value=’’;
$(‘api-key-input’).placeholder=‘✅ Chave salva’;
$(‘api-banner’).style.borderColor=‘var(–green)’;
showToast(‘✅ Chave salva! Clique em Atualizar Cotações.’);
}
...
if(!state.apiKey){showToast(‘⚠️ Configure sua chave brapi.dev!’);return;}
...
}else showToast(`⚠️ Sem dados para ${stock.ticker}`);
}catch(e){showToast(`Erro: ${stock.ticker}`);}
}

async function refreshOne(id){
if(!state.apiKey){showToast(‘⚠️ Configure sua chave!’);return;}
...
if(!state.stocks.length){tbody.innerHTML=`<tr><td colspan="12" style="text-align:center;padding:36px;color:var(--muted)">Nenhuma ação cadastrada.</td></tr>`;return;}
...
return`<tr> <td><span style="color:${c};font-weight:700;font-size:13px">${s.ticker}</span><br><span class="muted" style="font-size:10px">${s.name}</span></td> <td>${s.qty.toLocaleString('pt-BR')}</td> <td>R$ ${f2(s.pm)}</td> <td class="bold ${hasP?(s.price>=s.pm?'green':'red'):'muted'}" style="font-size:13px">${hasP?'R$ '+f2(s.price):'—'}</td> <td class="bold ${s.chg!=null?(s.chg>=0?'green':'red'):'muted'}">${s.chg!=null?fp(s.chg):'—'}</td> <td class="muted">${s.target?'R$ '+f2(s.target):'—'}</td> <td class="bold ${up!=null?(up>=0?'green':'red'):'muted'}">${up!=null?fp(up):'—'}</td> <td class="muted">R$ ${f2(inv)}</td> <td class="bold">${hasP?'R$ '+f2(atual):'—'}</td> <td class="bold ${hasP?(rBRL>=0?'green':'red'):'muted'}">${hasP?(rBRL>=0?'+':'−')+'R$ '+f2(abs(rBRL)):'—'}</td> <td class="bold ${hasP?(rPct>=0?'green':'red'):'muted'}">${hasP?fp(rPct):'—'}</td> <td style="white-space:nowrap"> <button class="icon-btn gold" onclick="refreshOne(${s.id})" title="Atualizar">↺</button> <button class="icon-btn muted" onclick="openEditModal(${s.id})" title="Editar">✏️</button> <button class="icon-btn" style="color:#ff6b6b55" onclick="removeStock(${s.id})" title="Remover">✕</button> </td> </tr>`;
}).join(’’);
}
...
/* ── MODAL ADD/EDIT ── */
function openAddModal(){editId=null;$(‘modal-title’).textContent=‘➕ Nova Ação’;[‘m-ticker’,‘m-name’,‘m-qty’,‘m-pm’].forEach(id=>$(id).value=’’);$(‘modal’).style.display=‘flex’;}
function openEditModal(id){
editId=id;const s=state.stocks.find(x=>x.id===id);
$(‘modal-title’).textContent=‘✏️ Editar Ação’;
...
if(!ticker||isNaN(qty)||isNaN(pm)||qty<=0||pm<=0){showToast(‘⚠️ Preencha todos os campos.’);return;}
if(editId){
...
gc.style.background=t===‘compra’?‘var(–green)’:‘var(–bg3)’;gc.style.color=t===‘compra’?’#07111f’:‘var(–muted)’;
rv.style.background=t===‘venda’?‘var(–red)’:‘var(–bg3)’;rv.style.color=t===‘venda’?’#07111f’:‘var(–muted)’;
...
if(opType===‘compra’&&exist){const nq=exist.qty+qty,np=(exist.pm*exist.qty+price*qty)/nq;html+=`<br>Novo PM: <span class="gold">R$${f2(np)}</span> | Total: <span style="color:var(--text)">${nq.toLocaleString('pt-BR')} ações</span>`;}
...
if(!ticker||isNaN(qty)||isNaN(price)||qty<=0||price<=0){msgEl.innerHTML=msg_(false,‘⚠️ Preencha todos os campos.’);return;}
const exist=state.stocks.find(s=>s.ticker===ticker);
let ok=true,txt=’’;
if(opType===‘compra’){
if(exist){const nq=exist.qty+qty,np=(exist.pm*exist.qty+price*qty)/nq;exist.qty=nq;exist.pm=+np.toFixed(4);exist.price=null;exist.chg=null;txt=`✅ +${qty} ${ticker} @ R$${f2(price)} | Novo PM: R$${f2(np)} | Total: ${nq.toLocaleString('pt-BR')} ações`;}
else{state.stocks.push({id:NID++,ticker,name:ticker,qty,pm:price,price:null,chg:null,target:null});txt=`✅ ${ticker} adicionado! ${qty} ações @ R$${f2(price)}`;}
}else{
if(!exist){ok=false;txt=`⚠️ Você não tem ${ticker}.`;}
else if(qty>exist.qty){ok=false;txt=`⚠️ Você só tem ${exist.qty} ações.`;}
...
const btn=$(‘btn-journal’);btn.disabled=true;btn.innerHTML=’<span class="spin">⟳</span> Gerando…’;
$(‘journal-date’).textContent=new Date().toLocaleDateString(‘pt-BR’,{weekday:‘long’,day:‘numeric’,month:‘long’,year:‘numeric’});
$(‘journal-content’).innerHTML=`<div class="spinning-wrap"><div class="spin" style="font-size:30px">⟳</div><div style="color:var(--muted);margin-top:16px;font-size:12px;line-height:2">Buscando notícias e relatórios...<br>Aguarde ~30 segundos.</div></div>`;
const tickers=state.stocks.map(s=>s.ticker).join(’, ‘);
try{
const res=await fetch(‘https://api.anthropic.com/v1/messages’,{method:‘POST’,headers:{‘Content-Type’:‘application/json’},body:JSON.stringify({model:‘claude-sonnet-4-20250514’,max_tokens:4000,tools:[{type:‘web_search_20250305’,name:‘web_search’}],messages:[{role:‘user’,content:`Você é um analista financeiro sênior. Pesquise notícias de hoje e gere um briefing em português com estas seções em MAIÚSCULAS:\n\nMERCADO HOJE\nResumo do Ibovespa: pontuação, variação, destaques.\n\nCENÁRIO BRASIL\nSELIC, inflação, câmbio, política fiscal.\n\nCENÁRIO GLOBAL\nFed, China, commodities, impactos no Brasil.\n\n${state.stocks.map(s=>`${s.ticker} - ${s.name.toUpperCase()}\nNotícias recentes, resultados financeiros com números, recomendação de analistas e preço-alvo.`).join('\n\n')}\n\nData: ${new Date().toLocaleDateString('pt-BR')}. Seja específico com números.`}]})});
const data=await res.json();
const text=(data.content||[]).filter(b=>b.type===‘text’).map(b=>b.text).join(’\n’).trim();
if(text&&text.length>300){renderJournal(text);}
else{$(‘journal-content’).innerHTML=`<div class="j-section" style="border-left:3px solid var(--red)"><div class="j-title" style="color:var(--red)">❌ Erro</div><p class="j-text">Tente novamente.</p></div>`;}
}catch(e){$(‘journal-content’).innerHTML=`<div class="j-section" style="border-left:3px solid var(--red)"><div class="j-title" style="color:var(--red)">❌ Erro de conexão</div><p class="j-text">${e.message}</p></div>`;}
btn.disabled=false;btn.textContent=‘📰 Gerar / Atualizar Jornal’;
}

function renderJournal(text){
const SEC_DEFS=[{key:‘MERCADO HOJE’,color:’#f0b429’,icon:‘📈’},{key:‘CENÁRIO BRASIL’,color:’#00d4ff’,icon:‘🇧🇷’},{key:‘CENÁRIO GLOBAL’,color:’#a78bfa’,icon:‘🌍’},…state.stocks.map((s,i)=>({key:s.ticker,color:COLORS[i%COLORS.length],icon:‘🏢’,stock:s}))];
const lines=text.split(’\n’);const sections=[];let cur=null;
for(const line of lines){const up=line.trim().toUpperCase();const def=SEC_DEFS.find(d=>up.startsWith(d.key));if(def){if(cur)sections.push(cur);cur={def,title:line.trim(),body:[]};}else if(cur)cur.body.push(line);}
if(cur)sections.push(cur);
if(!sections.length){$(‘journal-content’).innerHTML=`<div class="j-section"><p class="j-text">${text.replace(/\n/g,'<br>')}</p></div>`;return;}
$(‘journal-content’).innerHTML=sections.map(sec=>{
const bodyText=sec.body.join(’\n’).trim().replace(/\n/g,’<br>’);
if(sec.def.stock){const s=sec.def.stock,c=sec.def.color;return`<div class="j-company"><div class="j-company-head"><div class="j-chip" style="background:${c}22;border:1px solid ${c}44;color:${c}">${s.ticker}</div><div><div style="font-weight:700;color:#fff;font-size:14px">${s.name}</div><div style="font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Análise detalhada</div></div></div><div class="j-company-body"><p class="j-text">${bodyText}</p></div></div>`;}
...
/* ── RESET (para testes) ── */
function resetApp(){
if(!confirm(‘Apagar todos os seus dados e começar do zero?’))return;
